...
class TenPercentDiscountTest {
    ...
    @Test
    void shouldApply15PercentDiscountWhenAtLeast3GrainProductsThen10PercentDiscountWhenPriceIsAbove50() {
        // Given
        var productDb = new ProductDb();
        var cheese = productDb.getProduct("Cheese");
        var steak = productDb.getProduct("Steak");
        ...
        var expectedTotalPrice = cheese.price().add(steak.price()).add(bread.price()).add(bread.price()).add(cereals.price()).multiply(BigDecimal.valueOf(0.85)).multiply(BigDecimal.valueOf(0.9));

        // When
        var receiptAfterGrainDiscount = discount15.apply(receipt);
        var receiptAfterDiscounts = discount10.apply(receiptAfterGrainDiscount);
